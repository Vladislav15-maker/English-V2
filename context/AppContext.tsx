...
const initialState: AppState = {
  users: USERS, // Используем константы для начальной инициализации
  units: UNITS,
...
const appReducer = (state: AppState, action: Action): AppState => {
    switch (action.type) {
        ...
        case 'SET_INITIAL_STATE':
            // Перезаписываем только те части, которые пришли из облака, но сохраняем константы (USERS, UNITS, ONLINE_TESTS)
            return { 
                ...state, 
                ...action.payload, 
                users: USERS, 
        ...
        case 'SEND_MESSAGE': {
            ...
            // 1. Мгновенно отправляем сообщение через API для real-time доставки
            ...
            // 2. Обновляем локальное состояние отправителя (чтобы он увидел свое сообщение сразу)
            return {
                ...state,
        ...
        case 'RECEIVE_MESSAGE': {
            ...
            // Предотвращаем дублирование у отправителя (отправитель получает свое сообщение по локальному SEND_MESSAGE, а не по RECEIVE_MESSAGE)
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const reloadStateFromCloud = useCallback(async () => {
        ...
        try {
            ...
            // Здесь мы не используем SET_INITIAL_STATE, чтобы не перезаписывать константы (USERS, UNITS)
            // Мы просто обновляем нужные поля. Но для простоты используем SET_INITIAL_STATE с merged state.
            const mergedState = {
                ...initialState,
                ...cloudState,
                users: USERS,
    ...
    useEffect(() => {
        ...
        if (!pusherKey || !pusherCluster) {
            console.warn("Pusher keys are not defined in environment variables. Real-time updates will be disabled.");
            return;
        }
        ...
        try {
            ...
            // Слушаем НОВОЕ событие специально для сообщений чата
            ...
            console.error("Failed to initialize Pusher. Real-time will be disabled.", error);
        }
    }, [reloadStateFromCloud]);
    ...
    const saveStateToCloud = useCallback(() => {
        ...
        debounceTimer.current = setTimeout(() => {
            ...
            // Важно: Удаляем USERS, UNITS, ONLINE_TESTS, чтобы не перегружать облако константами
            ...
            .then(res => {
                if(res.ok) {
                    // Отправляем общий сигнал об обновлении, чтобы обновилось все, кроме чата
